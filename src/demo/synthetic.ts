...
import { buildGraph, normalizeRfc, type RiskGraph } from '../graph/adjacency.js';

/** The client under analysis. */
export const DEMO_ROOT: RFC = 'CLI010101AA1';

/** Only the listed RFCs; everything else is unlisted (weight 0). */
export const DEMO_SITUATIONS: Readonly<Record<RFC, Situacion69B>> = {
...
export const DEMO_EDGES: ReadonlyArray<{ de: RFC; a: RFC }> = [
...
export const demoWeightOf: WeightResolver = (rfc: RFC): number =>
  baseWeightFor(DEMO_SITUATIONS[normalizeRfc(rfc)] ?? null);

/** Builds the demo graph. */
export function buildDemoGraph(): RiskGraph {

...
export const BASE_WEIGHTS: Readonly<Record<Situacion69B, number>> = {
  Definitivo: 1.0,
  Presunto: 0.6,
  Desvirtuado: 0.05,
...
export function baseWeightFor(situacion: Situacion69B | null | undefined): number {
  if (!situacion) return 0;
 ...
 */
export function normalizeSituacion(raw: string): Situacion69B | null {
  const s = raw
    .normalize('NFD')
    .replace(/[̀-ͯ]/g, '')
    .trim()
    .toLowerCase();
  if (s.startsWith('definitiv')) return 'Definitivo';
  if (s.startsWith('presunt')) return 'Presunto';
  if (s.startsWith('desvirtu')) return 'Desvirtuado';
